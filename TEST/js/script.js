...
$items.each(function() {
	const $item = $items.find('.js-item');

	$item.each(function(index, item) {
		$(item).hover(function (){
			let i = 0;
			while (i <= $cards.length) {
				$cards.eq(index).find('.js-img').css('opacity', '1')			
				if (index > i) {
		...
		$(item).mouseleave(function () {
			$cards.eq(index).find('.js-img').css('opacity', '0')
		})
	})
});
...
const $hamburgerClose = $('.js-close');


$hamburgerClose.click(() => {
	$hamburgerClose.toggleClass('hamburger__close_active')
	$hamburger.toggleClass('hamburger__body_active')
})
